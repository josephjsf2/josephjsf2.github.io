...
所以最一開始那張沒有平衡的圖(假設紫色的節點是剛新增上去的新節點)，在新增節點後造成左邊 subtree過重，所以需要對 root做 right rotation，這張圖的情況判對為 LL型，如前面提到LL型定義：新增的紫色節點在root 的左節點的左節點，故只要做一次 right rotation即可：
